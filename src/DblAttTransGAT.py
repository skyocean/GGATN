...
# =========================================================
# Sequence Transformer encoder with attention extraction
# Contextualizes each sequence position through multi head
# self attention before graph grounded cross attention.
# =========================================================

class TransformerLayerPreLN(nn.Module):

    """
    One Pre LN Transformer self attention layer.
    Returns contextualized sequence states and, optionally,
    the self attention weights of this layer.
    """
...
class TransformerEncoderWithAttn(nn.Module):
    """
    Stack of Pre LN Transformer self attention layers.
    Returns the final sequence representation and either
    the last layer attention or all layer attentions.
    """
    ...
    def forward(
        ...
        for i, layer in enumerate(self.layers):
            ...
            x, attn = layer(x, key_padding_mask = key_padding_mask, need_weights = need)
...
class GraphCrossAttention(nn.Module):
    ...
    allow each sequence position to attend to graph encoded activity states, 
    then inject that signal conservatively through a learnable gate.
...
class GraphSequenceDoubleAttentionConfig:
    ...
    latent_dim: int = 0 # keep OFF; only on as integer to increase attribute diversity without hurt activity
...
class GraphSequenceDoubleAttentionOneShotModel(nn.Module):
    ...
    def __init__(self, cfg: GraphSequenceDoubleAttentionConfig):
        ...
        # sample a random vector z and injects it into the sequence condition
        # give the generator some stochastic variation beyond deterministic conditioning
        if cfg.latent_dim > 0:
    ...
    def _decode_activity_logits(
        ...
        ###----------#
        #each position representation H is scored by cosine similarity against every graph encoded activity prototype in E_act. 
        #H_final at position t, → compare with every graph encoded activity embedding→ activity with closer graph semantic match gets higher logit
        #graph enters decoding twice
        #1. graph cross attention:  H_seq attends to E_act and receives graph structured information
        #2. cosine prototype decoding: H_final is directly matched against E_act as graph encoded activity prototypes
    ...
    def _build_input(
        ...
        # the sequence_head layout is [length_norm, sin_hour, cos_hour, sin_wday, cos_wday]
        if drop_length_feature and x_case.shape[1] >= 1:
            x_case = x_case.clone()
            #Optionally removes the length feature
            x_case[:, 0] = 0.0
            
        # Projects sequence level features into d_model    
        cond = self.case_proj(x_case)  # (B, d)

        #Optionally adds latent noise
        ...
        #Broadcasts the same sequence condition to every position.
    ...
    def _build_activity_key_padding_mask(self, B: int, E_act: torch.Tensor) -> torch.Tensor:
        ...
        # true = masked, cannot be attended to
    ...
    def forward(
        ...
        # --------------------------------------------------
        # PASS 1: provisional activity decode
        # --------------------------------------------------
        # first produces provisional activity logits from the H_struct
        activity_logits_1 = self._decode_activity_logits(H_struct, E_act)
        
        # --------------------------------------------------
        # OPTIONAL ACTIVITY FEEDBACK REFINEMENT
        # forced model to confront its own provisional activity distribution and use it as additional context for a second refinement pass
        # --------------------------------------------------
        if self.use_activity_feedback:
            # feedback temperature
            tau = self.cfg.activity_feedback_temp
            act_logits_fb = activity_logits_1.clone()
            act_logits_fb[..., self.cfg.eos_id] = -1e9
            act_logits_fb[..., self.cfg.sos_id] = -1e9
            # turns first provisional logits into a soft activity distribution, due to their nosie
            act_probs_1 = F.softmax(act_logits_fb / tau, dim=-1)      # (B, L, A)
            # each position receives a weighted average of all activity embeddings, weighted by its provisional activity probabilities for uncertainty
            act_soft_emb_1 = torch.matmul(act_probs_1, E_act)         # (B, L, d)
        
            g_fb = torch.tanh(self.act_feedback_gate)
            #this soft activity embedding is projected and injected back into H_struct
            H_refine_in = self.act_feedback_norm(H_struct + g_fb * self.drop(self.act_feedback_proj(act_soft_emb_1)))
        
            H_final, refine_attn_list = self.refine_transformer(
                H_refine_in,
                key_padding_mask=key_padding_mask,
                return_all_attn=False
            )
        
            activity_logits = self._decode_activity_logits(H_final, E_act)
        else:
            H_final = H_struct
            refine_attn_list = []
            activity_logits = activity_logits_1

        if use_teacher_forcing_transition and ("y_activity" in batch):
            # adds a learned transition preference based on the previous true activity
            prev_tokens = self._build_prev_tokens_train(batch["y_activity"])
            # final logits:refined neural activity logits + teacher forced transition bias
            activity_logits = self._apply_transition_bias_train(activity_logits, prev_tokens)
        ...
        # default: teacher forcing for attributes
        # --------------------------------------------------
        ...
        act_emb_attr = E_act[act_ids_attr]   # (B, L, d)

        # event-level heads use final hidden state + chosen activity embedding
        event_attr_in = torch.cat([H_final, act_emb_attr], dim=-1)

        # time now uses the same aligned input
        ...
        case_emb_attr = masked_mean(H_final, seq_pool_mask, dim=1)
    ...
    def generate(
        ...
        greedy: bool = True, # determinitic mode
        ...
        # determine lengths early if provided
        if length_override is not None:
            real_lengths = length_override.to(device).long().clamp(min=1, max=L - 1)
            total_lengths = real_lengths + 1
            arange = torch.arange(L, device=device).unsqueeze(0)
            full_mask = (arange < total_lengths.unsqueeze(1)).float()
        else:
        ...
        out = self.forward(
            ...
            return_graph_attn=return_attn,
            use_teacher_forcing_transition=False,
            use_teacher_forcing_attributes=False
        )
    
        act_logits = torch.nan_to_num(
            out["activity_logits"].clone(),
            nan=-1e9,
            posinf=1e9,
            neginf=-1e9
        )
        ...
        # --------------------------------------------------
        # 2. determine real-event lengths
        # --------------------------------------------------
        if length_override is None:
            len_logits = out["length_logits"]
            if greedy:
                total_lengths = (len_logits.argmax(dim=-1) + 1).clamp(2, L)
            else:
                len_probs = F.softmax(len_logits / max(temperature, 1e-8), dim=-1)
                total_lengths = (torch.multinomial(len_probs, 1).squeeze(1) + 1).clamp(2, L)
    
            real_lengths = (total_lengths - 1).clamp(min=1, max=L - 1)

    
        # --------------------------------------------------
        # 3. build pairwise transition scores
        # --------------------------------------------------
        pair_scores = torch.zeros((A, A), device=device)
    
        if self.use_transition_bias and self.transition_bias is not None:
            # Detach to make sure decoding procedure does not track gradients through the transition bias
            pair_scores = pair_scores + self.cfg.transition_bias_weight * self.transition_bias.detach().to(device)
    
        if bigram_prior is not None:
            # transition frequency prior from trained dataset
        ...
        pair_scores[pad_id, :] = -1e9
        pair_scores[unk_id, :] = -1e9
        pair_scores[eos_id, :] = -1e9   # EOS has no outgoing transitions

        # restrict transition scores to the global process graph
        if adj_matrix is not None:
            adj = adj_matrix.to(device).float()
            valid = adj > 0
    
            # allow transitions to EOS even if adjacency matrix omitted them
            valid[:, eos_id] = True
    
            pair_scores = torch.where(valid, pair_scores, torch.full_like(pair_scores, -1e9))
    
        # start transition from sos -> first event
        start_scores = torch.zeros((A,), device=device)
        if self.use_transition_bias and self.transition_bias is not None:
            start_scores = start_scores + self.cfg.transition_bias_weight * self.transition_bias[sos_id].detach().to(device)
    
        if bigram_prior is not None:
            start_scores = start_scores + torch.log(bigram[sos_id].clamp_min(1e-9))
    
        start_scores[pad_id] = -1e9
        start_scores[unk_id] = -1e9
        start_scores[eos_id] = -1e9
        start_scores[sos_id] = -1e9

    
        # --------------------------------------------------
        # 4. Viterbi decoding per case
        # --------------------------------------------------

        #create the final generated activity tensor with pad in every position
        y_act = torch.full((B, L), pad_id, dtype=torch.long, device=device)
    
        for b in range(B):
            T = int(real_lengths[b].item())   # number of real events
    
            unary = act_logits[b, :T, :].clone()   # (T, A)
    
            unary[:, pad_id] = -1e9
            unary[:, unk_id] = -1e9
            unary[:, eos_id] = -1e9  # EOS forbidden on real-event positions
            unary[:, sos_id] = -1e9
    
            if top_k is not None and top_k < A:
                #For each position t, this finds the Top k activity scores
                vals, idx = torch.topk(unary, top_k, dim=-1)
                filtered = torch.full_like(unary, -1e9)
                filtered.scatter_(1, idx, vals)
                unary = filtered
    
            # optional temperature smoothing before decoding
            if not greedy and temperature != 1.0:
                unary = unary / max(temperature, 1e-8)
    
            # dynamic programming score table
            dp = torch.full((T, A), -1e9, device=device)
            back = torch.full((T, A), -1, dtype=torch.long, device=device)
    
            # t = 0
            dp[0] = self.cfg.unary_weight * unary[0] + start_scores
    
            # t = 1..T-1
            for t in range(1, T):
                scores = dp[t - 1].unsqueeze(1) + pair_scores   # (A_prev, A_curr)
                best_scores, best_prev = scores.max(dim=0)
    
                dp[t] = self.cfg.unary_weight * unary[t] + best_scores
                back[t] = best_prev
    
            # terminal transition to EOS
            end_scores = dp[T - 1] + pair_scores[:, eos_id]
            last_token = end_scores.argmax()
    
            # backtrack for path
            path = [last_token.item()]
            for t in range(T - 1, 0, -1):
                last_token = back[t, last_token]
                path.append(last_token.item())
            path.reverse()
    
            # write real events
            y_act[b, :T] = torch.tensor(path, dtype=torch.long, device=device)
        ...
        # --------------------------------------------------
        arange = torch.arange(L, device=device).unsqueeze(0)
        gen_mask = arange < total_lengths.unsqueeze(1)              # includes EOS
        non_eos_gen_mask = gen_mask & (y_act != eos_id)
    
        lengths = total_lengths.clone()
        real_event_lengths = real_lengths.clone()
    
        # --------------------------------------------------
        # 6. recompute event inputs from final decoded path
        # --------------------------------------------------
        act_emb_attr = E_act[y_act] * gen_mask.unsqueeze(-1)
        event_attr_in = torch.cat([out["H"], act_emb_attr], dim=-1)

        # --------------------------------------------------
        # 7. time decoding from final decoded path
        # --------------------------------------------------
        ...
        case_emb_attr = masked_mean(out["H"], non_eos_gen_mask.float(), dim=1)
        ...
        result = {
            "lengths": lengths,                         # includes EOS
            "real_event_lengths": real_event_lengths,   # excludes EOS
            "mask": gen_mask,
        ...
        if return_attn:
            result["transformer_attn"] = out["transformer_attn"]
            result["graph_attn"] = out["graph_attn"]
...
def invert_numeric(arr, meta):
    """
    Inverse transform for numeric predictions.
    If meta["positive"] is True, undo z-score in log1p space,
    then apply expm1 and clamp to >= 0.
    """
    out = arr * meta["std"] + meta["mean"]
...
def generate_full_log(
    ...
    df_events = attach_event_attributes(
        ...
        event_num_post={
            c: (lambda arr, meta=num_stats["event"][c]: invert_numeric(arr, meta))
    ...
    df_cases = decode_sequence_attributes(
        ...
        seq_num_post={
            c: (lambda arr, meta=num_stats["seq"][c]: invert_numeric(arr, meta))
